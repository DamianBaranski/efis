/// \file europe_map.h
/// Schematic Europe chart used by the ENR charts page.
#ifndef EUROPE_MAP_H
#define EUROPE_MAP_H

#include "europe_borders.h"
#include "render2d.h"
#include "screen.h"
#include "shader.h"

#include <vector>

/// Schematic Europe map drawn from simplified country rings. Seas are the panel.
class EuropeMap
{
public:
    /// Allocates the land and border meshes. Call layout before the first draw.
    /// \param screen Window the chart is drawn into.
    explicit EuropeMap(Screen &screen);

    /// Fits the chart in the page rectangle. Call again when the window changes.
    /// Coordinates are SDL pixels, origin at the top left.
    void layout(int contentX, int contentY, int contentW, int contentH, int screenW, int screenH);
    /// Draws land, borders, and the country name under the last hit.
    void render();
    /// Selects the country under the point.
    /// \return True when the point is inside the chart.
    bool hit(int sdlX, int sdlY);

private:
    struct Point
    {
        float x = 0.0f;
        float y = 0.0f;
    };

    static VertexTexture vert(float x, float y, float u, float v);
    void project();
    void rasterFill();
    void rebuildQuad();
    void rebuildStroke();
    void rebuildLabel();
    void setOrtho();
    int pickCountry(float glX, float glY) const;
    static bool ringContains(float x, float y, const Point *pts, int count);
    static bool clipWall(int vertexA, int vertexB);

    Shader mLand;
    Shader mStroke;
    Render2D mLabel;
    std::vector<Point> mProj;
    glm::mat4 mMvp{1.0f};
    float mMapX = 0.0f;
    float mMapY = 0.0f;
    float mMapW = 1.0f;
    float mMapH = 1.0f;
    int mContentX = 0;
    int mContentY = 0;
    int mContentW = 0;
    int mContentH = 0;
    int mScreenW = 0;
    int mScreenH = 0;
    int mSelected = -1;
    bool mLabelOn = false;
    bool mReady = false;
};

#endif
